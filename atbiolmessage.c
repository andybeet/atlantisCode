...
static FILE *vbiomAnnAgeNumfp;
static FILE *vtempPopRatiofp;
...
static FILE * Init_AnnualAgeNum_File(MSEBoxModel *bm);
static FILE * Init_tempPopRatio_File(MSEBoxModel *bm);
...
static void Write_AnnualAgeNUMBER(FILE *fid, MSEBoxModel *bm);
static void Write_tempPopRatio(FILE *fid, MSEBoxModel *bm);
...
void Open_Ecology_Output_Files(MSEBoxModel *bm) {
	...
	if (bm->flag_age_output) {
        ...
        vbiomAnnAgefp = Init_AnnualAgeBiom_File(bm);
				vbiomAnnAgeNumfp = Init_AnnualAgeNum_File(bm);
				vtempPopRatiofp = Init_tempPopRatio_File(bm);
    }
...
void Close_Ecology_Output_Files(MSEBoxModel *bm) {
    ...
    if (bm->flag_age_output) {
        ...
        Util_Close_Output_File(vbiomAnnAgefp);
				Util_Close_Output_File(vbiomAnnAgeNumfp);
				Util_Close_Output_File(vtempPopRatiofp);
    }
	if(bm->lim_sun_hours){
...
static void Write_Biomass(MSEBoxModel *bm) {
	...
	if(bm->flag_age_output){
        ...
        Write_AnnualAgeBiomass(vbiomAnnAgefp, bm);
				Write_AnnualAgeNUMBER(vbiomAnnAgeNumfp, bm);
				Write_tempPopRatio(vtempPopRatiofp, bm);
	}
...
/****************************************************************************************************************
 *	\brief Routines to initialise the age structured NUMBERS information file - from Savina model
 * ADDED BY A.BEET
 */
static FILE * Init_AnnualAgeNum_File(MSEBoxModel *bm) {
    FILE *fid;
    ...
    /** Create filename **/
    sprintf(fname, "%sAnnualAgeNumbersIndx.txt", bm->startfname);
    printf("Creating %s\n", fname);
    
    /** Create file **/
    if ( (fid=Util_fopen(bm, fname, "w")) == NULL )
        quit("Init_AnnualAgeNum_File: Can't open %s\n",fname);
...
/**
 *	\brief Routines to write data to the Age structrured NUMBERS information file - now in t, used to be in mg N
 * ADDED BY A.BEET
 */
static void Write_AnnualAgeNUMBER(FILE *fid, MSEBoxModel *bm) {
    int sp, cohort, this_cohort, i, k;
    ...
    for (sp = 0; sp < bm->K_num_tot_sp; sp++) {
        if ( (int)(FunctGroupArray[sp].speciesParams[flag_id])){
            for (i = 0; i < FunctGroupArray[sp].numStocks; i++) {
                ...
                for (cohort = 0; cohort < FunctGroupArray[sp].numCohortsXnumGenes; cohort++) {
                    for (k = 0; k < FunctGroupArray[sp].ageClassSize; k++) {
                        fprintf(fid, " %e", bm->totagepop[sp][cohort] * bm->tempPopRatio[i][sp][cohort][k]);
                    }
                    this_cohort++;
                }
            }
...
/****************************************************************************************************************
 *	\brief Routines to initialise the tempPopRatio file - from Savina model
 * ADDED BY A.BEET
 */
static FILE * Init_tempPopRatio_File(MSEBoxModel *bm) {
    FILE *fid;
    ...
    /** Create filename **/
    sprintf(fname, "%stempPopRatio.txt", bm->startfname);
    printf("Creating %s\n", fname);
    
    /** Create file **/
    if ( (fid=Util_fopen(bm, fname, "w")) == NULL )
        quit("Init_tempPopRatio_File: Can't open %s\n",fname);
...
/**
 *	\brief Routines to write data to the tempPopRatio file - now in t, used to be in mg N
 * ADDED BY A.BEET
 */
static void Write_tempPopRatio(FILE *fid, MSEBoxModel *bm) {
    int sp, cohort, this_cohort, i, k;
    ...
    if (verbose > 1)
        fprintf(stderr, "Write tempPopRatio information\n");
    ...
    for (sp = 0; sp < bm->K_num_tot_sp; sp++) {
        if ( (int)(FunctGroupArray[sp].speciesParams[flag_id])){
            for (i = 0; i < FunctGroupArray[sp].numStocks; i++) {
                ...
                for (cohort = 0; cohort < FunctGroupArray[sp].numCohortsXnumGenes; cohort++) {
                    for (k = 0; k < FunctGroupArray[sp].ageClassSize; k++) {
                        fprintf(fid, " %e", bm->tempPopRatio[i][sp][cohort][k]);
                    }
                    this_cohort++;
                }
            }
