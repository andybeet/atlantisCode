    ...
    for (cohort = ((FunctGroupArray[species].numCohortsXnumGenes) - 1); cohort >= 0; cohort--) {
        ...
        if(FunctGroupArray[species].scaled_FSPB[cohort] > 0.0) {
            ...
            if (semelparous_parents) {
                //fprintf(llogfp,"Time: %e %s-%d has DEN: %e and shiftDEN: %e ", bm->dayt, FunctGroupArray[species].groupCode, cohort, VERTinfo[species][cohort][DEN_id], shiftVERT[species][cohort][DEN_id]);
                ...
                /* If not at same time aging then need to update the shift values */
                shiftVERT[species][cohort][DEN_id] = (1.0 - FunctGroupArray[species].scaled_FSPB[cohort]) * VERTinfo[species][cohort][DEN_id];
                shiftVERTON[species]++;
			...
			if ((bm->current_box == bm->first_box) && (bm->current_layer == bm->top_layer)) {
				...
				for (mid = MIGRATION[species].num_in_queue_done; mid < MIGRATION[species].num_in_queue; mid++) {
                    ...
                    **/
                    ...
                    /**/
                    if(do_debug)
                     fprintf(bm->logFile, "Time: %e %s-%d mid: %d recruit_outside = %d, MIGRATION.recruit= %e\n",  bm->dayt, FunctGroupArray[species].groupCode, ngene, mid, recruit_outside, MIGRATION[species].recruit[ngene][mid]);
                     /**/
					...
					if (recruit_outside) {
						...
						if(MIGRATION[species].recruit[ngene][mid] > 0.0) {
							MIGRATION[species].DEN[ngene][mid] += MIGRATION[species].recruit[ngene][mid]; // As all in youngest cohort maps to ngene not cohort in Migration
...
void Find_Final_Recruit_Distribution(MSEBoxModel *bm, int species, double enviro_scalar, double vertdistrib, int wclayer, int stock_id, int do_debug, FILE *llogfp) {
    ...
    //if((species == 68) || (species == 69) || (species == 64)) {
	...
	for(ngene = 0; ngene < sp_numGeneTypes; ngene++){
        ...
        **/
		...
		case at_parent_location:
            ...
            /**
            if (do_debug && (species == bm->which_check)) {
                fprintf(llogfp,"Time: %e, %s box%d-%d ngene: %d, qid: %d, totrecruit: %e num_rec: %e (Bulkrecruits: %e, enviro_scalar: %e, vdistrib: %e, hdistrib: %e)\n", bm->dayt, FunctGroupArray[species].groupCode, bm->current_box, wclayer, ngene, qid, EMBRYO[species].num_recruits[bm->current_box][wclayer][ngene][qid], totrecruit[species][stock_id][ngene], EMBRYO[species].BulkRecruits[ngene], enviro_scalar, vertdistrib, bm->recruit_hdistrib[ngene][bm->current_box][species]);
            }
            **/
...
void Store_Recruitment_Diagnostics(MSEBoxModel *bm, int species, int use_aggregate, int do_debug, FILE *llogfp) {
	...
	double KWRR_sp = FunctGroupArray[species].speciesParams[KWRR_id];
	int sp_numGeneTypes = (int) (FunctGroupArray[species].numGeneTypes);
	int ngene = 0, i = 0, stock_id = 0, den, k;
	int fished_chrt = 0;
    ...
    *recruits_arrive = 0;
	...
	for (ngene = 0; ngene < sp_numGeneTypes; ngene ++) {
        ...
        if (do_debug && (bm->which_check == species) && (bm->dayt >= bm->checkstart)) {
			fprintf(llogfp, "Time: %e, %s get settlers ngene: %d, StartDay: %d, EndDay: %d, recruit sp_checkday: %d larvae starts: %e\n",
					bm->dayt, FunctGroupArray[species].groupCode, ngene, EMBRYO[species].StartDay[ngene][EMBRYO[species].next_recruit],
					EMBRYO[species].EndDay[ngene][EMBRYO[species].next_recruit], sp_checkday, EMBRYO[species].Larvae[stock_id][ngene][EMBRYO[species].next_recruit]);
		}
        **/
		...
		if ((sp_checkday >= EMBRYO[species].StartDay[ngene][EMBRYO[species].next_recruit]) && (sp_checkday < EMBRYO[species].EndDay[ngene][EMBRYO[species].next_recruit])) {
            ...
            /*
            if (do_debug && (bm->which_check == species)) {
				fprintf(llogfp, "Time: %e, %s ngene: %d, qid: %d StartDay: %d, EndDay: %d, sp_checkday: %d, recruitSPden: %e, remaining larvae: %e, num_recruits%d-%d: %e, temprec: %e, RecruitPeriod: %e, flagtrecruitdistrib: %d\n",
...
double Get_Numbers_Aging(MSEBoxModel *bm, int species, int cohort, int *active_den, double *lostden_zero, int do_debug, FILE *llogfp) {
	...
	if (bm->flag_do_evolution && bm->flagagestruct) {
        ...
        **/
        ...
        /**
		if (do_debug && (bm->which_check == species)) {
			fprintf(llogfp, "Time: %e, %s-%d shiftVERT[den]: %e, p_ageup: %e, VERTinfo[den]: %e (shiftVERTON: %d, dennow: %e)\n", bm->dayt,
				FunctGroupArray[species].groupCode, cohort, shiftVERT[species][cohort][DEN_id], p_ageup, VERTinfo[species][cohort][DEN_id], shiftVERTON[species], dennow);
		}
        **/
	}
				...
				if (sp_mignum) {
					for (cohort = (((FunctGroupArray[species].numCohorts - 1) * FunctGroupArray[species].numGeneTypes) - 1); cohort > -1 ; cohort--) { // Running backwards so don't need embedded stage check to see different migration status for mig_status (if run forward then get wrong answer for young_in_old_out as not done the later stages when do the younger
						...
						for (qid = MIGRATION[species].num_in_queue_done; qid < MIGRATION[species].num_in_queue; qid++) {
							...
							MIGRATION[species].prop_mig[cohort][qid] = MIGRATION[species].DEN[nextcid][qid] / (totden[species][cohort] + MIGRATION[species].DEN[nextcid][qid]
									+ small_num);
							...
							for (k = 0; k < bm->boxes[ij].nz; k++) {
								...
								if ((do_debug) || (!_finite(bm->boxes[ij].tr[k][den])) || (bm->boxes[ij].tr[k][den] < 0.0)) {
									...
									fprintf(llogfp, "Time: %e, box: %d-%d %s-chrt%d den: %e (dennow: %e, agingDEN: %e, AGEnewden: %e, stockratio: %e)\n",
											...
											spSTOCKprop[stock_id][cohort] / AGE_stock_struct_prop[species][cohort][stock_id]);
									fprintf(llogfp, "Time: %e, box%d-%d %s-chrt%d, denratio: %e, snratio: %e, rnratio: %e\n", bm->dayt, ij, k,
											FunctGroupArray[species].groupCode, cohort, bm->boxes[ij].tr[k][den] / (dennow + small_num), bm->boxes[ij].tr[k][sn]
													/ snnow, bm->boxes[ij].tr[k][rn] / rnnow);
								}
